      ...
      const patchKeyedChildren = (c1, c2, container, parentAnchor, parentComponent, parentSuspense, isSVG, optimized) => {
          ...
          // i = 2, e1 = 4, e2 = 5
              ...
              let n2 = (c2[e2] = optimized
                  ? cloneIfMounted(c2[e2])
                  : normalizeVNode(c2[e2]));
              let hasPrePatched = false;
              let c1PrePatchCount = 0;
              while (n1.key != null &&
                  n1.key === n2.key) {
                  newIndexToOldIndexMap[e2 - s2] = s1 + 1;
                  if (c1PrePatchCount === 0) {
                      c1PrePatchCount++;
                  }
                  else {
                      moved = true;
              ...
              if (s1 <= e1 && s2 <= e2 && patched < toBePatched) {
                  n1 = c1[e1];
                  n2 = (c2[s2] = optimized
                      ? cloneIfMounted(c2[s2])
                      : normalizeVNode(c2[s2]));
                  let c2PrePatchCount = 0;
                  while (n1.key != null &&
                      n1.key === n2.key) {
              ...
              for (i = s1; i <= e1; i++) {
                  ...
                  if (prevChild.key != null) {
                      ...
                      for (j = s2; j <= unKeyedNewChildIndexArr.length; j++) {
                          ...
                          if (newIndexToOldIndexMap[unKeyedIndex - c2StartIndex] === 0 &&
                              isSameVNodeType(prevChild, c2[unKeyedIndex])) {
              ...
              window.performance.mark("mySetTimeout-end");
              window.performance.measure(
                "mySetTimeout",
                "mySetTimeout-start",
                "mySetTimeout-end"
              );
              const measures = performance.getEntriesByName("mySetTimeout");
              const measure = measures[0];
              if (window.fastestTime) {
                if (window.fastestTime > measure.duration) {
                  window.fastestTime = measure.duration
                }
              } else{
                window.fastestTime = measure.duration
              }
              if (window.slowestTime) {
                if (window.slowestTime < measure.duration) {
                  window.slowestTime = measure.duration
                }
              } else {
                window.slowestTime = measure.duration
              }
              window.Count = window.Count ? window.Count + 1 : 1
              window.totalTime = (window.totalTime ? window.totalTime : 0) + measure.duration
